    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(command.getName().equalsIgnoreCase("helpme")){
            if(args.length < 1) {
                ...
                sender.sendMessage(ChatColor.YELLOW + "To set up a sign shop, you place a sign and in the first row write [T] - [your name], on the second row you put the item id you want to sell (you can find this out with /sellids) and next to it the amount per trade. On the third row you would do the same except for the item you want to recieve in the trade. On the last row you just put 0/0, this represents the stock of the item youre selling and the amount of items youve sold. You can add to this by clicking on your own sign with the item in your inventory. an Example would look like this: \n [TRADE] - Bkamp \n IRON_INGOT, 5 \n DIAMOND, 1 \n 0");
                return true;
            } else {
